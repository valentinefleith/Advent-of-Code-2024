...
fn is_combination(test: &u64, remaining: &Vec<u64>, current_result: &u64, n: usize) -> bool {
    if n == remaining.len() {
    ...
    is_combination(test, &remaining, &(current_result + remaining[n]), n + 1)
        || is_combination(test, &remaining, &(current_result * remaining[n]), n + 1)
        || is_combination(
            test,
            &remaining,
            &concatenate(&current_result, &remaining[n]),
            n + 1,
        )
}
...
pub fn get_right_combinations2(tested: Vec<u64>, remaining: Vec<Vec<u64>>) -> u64 {
    ...
    for (i, test) in tested.iter().enumerate() {
        if is_combination(test, &remaining[i], &remaining[i][0], 1) {
            result += test;
        }
    }
    result
